...
required_files = ["CASE_" + CASE + "_BUS DATA.csv", "CASE_" + CASE + "_BRANCH DATA.csv", "CASE_" + CASE + "_GEN DATA.csv"]
...
# Read Files
Bus_Data    = pd.read_csv(os.path.join(data_dir, "CASE_" + CASE + "_BUS DATA.csv")).to_numpy()
...
if not np.array_equal(bus_ids, expected_ids):
    raise ValueError("Bus Numbers are not sequential (1 to N). Re-indexing is required.")           # Check if Bus number is sequentially alocated or not

Branch_Data = pd.read_csv(os.path.join(data_dir, "CASE_" + CASE + "_BRANCH DATA.csv")).to_numpy()
Gen_Data    = pd.read_csv(os.path.join(data_dir, "CASE_" + CASE + "_GEN DATA.csv")).to_numpy()
...
temp[:N_pq]          = Bus_Data[pq_mask,
                                0]
temp[N_pq:N_pq+N_pv] = Bus_Data[pv_mask,
                                0]
temp[N_pq+N_pv:]     = Bus_Data[pq_mask,
                                0]
t = tuple(temp)                           # Mapping tuple for variable vector 'x' ... ( PQ - PV )

S_base = 100                              # Base power for per-unit analysis [MVA]
Y_bus  = admittance_matrix(N_bus,
                           Bus_Data,
                           Branch_Data,
                           S_base)        # Admittance Matrix
...
x0 = np.zeros_like( x )
x0 = Approx_starting_point(Y_bus=Y_bus, Bus_Data=Bus_Data, Gen_Data=Gen_Data,
                           S_base=S_base, N_bus=N_bus, N_pq=N_pq, N_pv=N_pv,
                           map_tuple=t, dim=dim, iterations=0)
...
for i in range(max_iter):
    ...
    J = Jacobian( map_tuple=t, Y_bus=Y_bus, V=V,
                 N_bus=N_bus, dim=dim, N_pq=N_pq, N_pv=N_pv)
    f = Mismatch( map_tuple=t, Y_bus=Y_bus, V=V, Bus_Data=Bus_Data, Gen_Data=Gen_Data,
                 N_bus=N_bus, dim=dim, N_pq=N_pq, N_pv=N_pv, S_base=S_base )
...
##### 5. Print Results #####
for i in range(N_bus):
    print(f"Bus No. {i+1}: Magnitude = {V[i, 0]:.4f} [p.u.], Angle = {V[i, 1]*180/np.pi:.4f} [deg]")